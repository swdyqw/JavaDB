...
import top.lzxu.javadb.backend.im.BPlusTree;
import top.lzxu.javadb.backend.parser.statement.SingleExpression;
...
public class Field {
    ...
    public static Field loadField(Table tb, long uid) {
        byte[] raw = null;
        try {
            raw = ((top.lzxu.javadb.backend.tbm.TableManagerImpl)tb.tbm).vm.read(TransactionManagerImpl.SUPER_XID, uid);
        } catch (Exception e) {
            Panic.panic(e);
        }
        assert raw != null;
    ...
    private Field parseSelf(byte[] raw) {
        ...
        if(index != 0) {
            try {
                bt = BPlusTree.load(index, ((top.lzxu.javadb.backend.tbm.TableManagerImpl)tb.tbm).dm);
            } catch(Exception e) {
                Panic.panic(e);
            }
        }
    ...
    public static Field createField(Table tb, long xid, String fieldName, String fieldType, boolean indexed) throws Exception {
        ...
        if(indexed) {
            long index = BPlusTree.create(((top.lzxu.javadb.backend.tbm.TableManagerImpl)tb.tbm).dm);
            BPlusTree bt = BPlusTree.load(index, ((top.lzxu.javadb.backend.tbm.TableManagerImpl)tb.tbm).dm);
            f.index = index;
            f.bt = bt;
        }
        f.persistSelf(xid);
    ...
    private void persistSelf(long xid) throws Exception {
        ...
        this.uid = ((top.lzxu.javadb.backend.tbm.TableManagerImpl)tb.tbm).vm.insert(xid, Bytes.concat(nameRaw, typeRaw, indexRaw));
    }
    ...
    public top.lzxu.javadb.backend.tbm.FieldCalRes calExp(SingleExpression exp) throws Exception {
        Object v = null;
        top.lzxu.javadb.backend.tbm.FieldCalRes res = new top.lzxu.javadb.backend.tbm.FieldCalRes();
