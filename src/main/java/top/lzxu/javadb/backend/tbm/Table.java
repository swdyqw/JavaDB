...
import top.lzxu.javadb.backend.tbm.Field.ParseValueRes;
import top.lzxu.javadb.backend.tm.TransactionManagerImpl;
import top.lzxu.javadb.backend.utils.Panic;
import top.lzxu.javadb.backend.utils.ParseStringRes;
 ...
 */
public class Table {
    top.lzxu.javadb.backend.tbm.TableManager tbm;
    long uid;
    String name;
    byte status;
    long nextUid;
    ...
    public static Table loadTable(top.lzxu.javadb.backend.tbm.TableManager tbm, long uid) {
        byte[] raw = null;
        try {
            raw = ((top.lzxu.javadb.backend.tbm.TableManagerImpl)tbm).vm.read(TransactionManagerImpl.SUPER_XID, uid);
        } catch (Exception e) {
            Panic.panic(e);
        }
        assert raw != null;
    ...
    public static Table createTable(top.lzxu.javadb.backend.tbm.TableManager tbm, long nextUid, long xid, Create create) throws Exception {
    ...
    public Table(top.lzxu.javadb.backend.tbm.TableManager tbm, long uid) {
        this.tbm = tbm;
        this.uid = uid;
    }

    public Table(top.lzxu.javadb.backend.tbm.TableManager tbm, String tableName, long nextUid) {
        this.tbm = tbm;
        this.name = tableName;
    ...
    private Table persistSelf(long xid) throws Exception {
        ...
        uid = ((top.lzxu.javadb.backend.tbm.TableManagerImpl)tbm).vm.insert(xid, Bytes.concat(nameRaw, nextRaw, fieldRaw));
        return this;
    }
    ...
    public int delete(long xid, Delete delete) throws Exception {
        ...
        int count = 0;
        for (Long uid : uids) {
            if(((top.lzxu.javadb.backend.tbm.TableManagerImpl)tbm).vm.delete(xid, uid)) {
                count ++;
            }
        }
        return count;
    ...
    public int update(long xid, Update update) throws Exception {
        ...
        int count = 0;
        for (Long uid : uids) {
            byte[] raw = ((top.lzxu.javadb.backend.tbm.TableManagerImpl)tbm).vm.read(xid, uid);
            if(raw == null) continue;

            ((top.lzxu.javadb.backend.tbm.TableManagerImpl)tbm).vm.delete(xid, uid);

            Map<String, Object> entry = parseEntry(raw);
            entry.put(fd.fieldName, value);
            raw = entry2Raw(entry);
            long uuid = ((top.lzxu.javadb.backend.tbm.TableManagerImpl)tbm).vm.insert(xid, raw);
    ...
    public void insert(long xid, Insert insert) throws Exception {
        ...
        long uid = ((top.lzxu.javadb.backend.tbm.TableManagerImpl)tbm).vm.insert(xid, raw);
        for (Field field : fields) {
            if(field.isIndexed()) {
